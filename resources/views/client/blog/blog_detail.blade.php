...
@section('content')
    <div class="inn-banner">
        <div class="container">
            <div class="row">
                <h4>{{ __('label.post.detail_title') }}</h4>
                <p>
                    {{ __('label.post.intro_page') }}
                </p>
            </div>
        </div>
    </div>
            ...
            <div class="row inn-page-com">
                ...
                <div class="col-md-8">
                    <div class="row inn-services in-blog">
                        <div class="col-md-12"><img id="img-blog"
                                                    src="{{ asset(config('common.uploads.posts')) . '/' . $data->image }}"
                                                    alt=""/></div>
                        <div class="col-md-12">
                            ...
                            @foreach($sameCategory as $value)
                                <li>
                                    <div class="rec-po-img"><img
                                                ...
                                                alt=""/></div>
                                    <div class="rec-po-title"><a href="{{ route('post.detail', $value->id) }}">
                                            <h4> {{ $value->title }}</h4></a>
                                        <p> {{ $value->description }} </p> <span
                                                class="blog-date">Date: {{ $value->updated_at }}</span></div>
                                </li>
                            @endforeach
                        </ul>
                    </div>
