...
@section('script')
    <script>
            ...
            init: function () {
                ...
                t = $(".m_datatable").mDatatable({
                    data: {
                        ...
                        source: {
                            read: {
                                url: "{{ route('admin.units.datatable') }}",
                                method: 'GET',
                                map: function (t) {
                                    let e = t;
                        ...
                        },
                        {
                            ...
                            template: function (e) {
                                if (e.lang_parent_id == 0) {
                                    ...
                                    return `<a href="{{ route('admin.units.origin', '') }}/${e.lang_parent_id}">Xem bản gốc</a>`;
                                }
                            }
                        },
                        {
                            ...
                            template: function (e, a, i) {
                                ...
                                const edit = `<a href="{{ route('admin.units.edit', '') }}/${translateId}"
                                                       class="m-portlet__nav-link btn m-btn m-btn--hover-info m-btn--icon m-btn--icon-only m-btn--pill"
                                                       title="Chỉnh sửa"
                                                    >
                                                        <i class="la la-edit"></i>
                                                    </a>`;
                                const translate = ` <a href="{{ route('admin.units.translation', '') }}/${translateId}"
                                ...
                                return `${edit} ${translate} ${deleteUnit}`
                            }
                        }
                    ]
        ...
        function deleteUnit(t) {
            let id = $(t).attr('unitId');
            swal({
                ...
                e.value && $.ajax({
                    contentType: false,
                    processData: false,
                    url: `{{ route('admin.units.delete', '') }}/${id}`,
                    type: 'POST',
                    success: function (response) {
                        ...
                        if (response.messages == 'used') {
                            toastr.error('Có dịch vụ đang sử dụng đơn vị này', 'Thất bại');
                        }
